...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\yunwa\\OneDrive\\Documents\\GSN-LMU MSC NEURO 2021\\Research Projects\\NEVIA Lab (LMU Klinikum)\\MAPC Project\\stress_MAPC_task_1\\stress_search_Deutsch_lastrun.py',
...
# --- Initialize components for Routine "synchronize" ---
text = visual.TextStim(win=win, name='text',
    text='um die Uhr mit dem Programm zu synchronisieren:\n\nDrücken Sie die LEERTASTE und die Uhr gleichzeitig.',
    font='Open Sans',
    pos=(0, 0), height=1.0, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
    depth=0.0);
key_sync = keyboard.Keyboard()
...
# create target color, and target orientation [0,1]
...
# add cue validity as one column
...
block_info = visual.TextStim(win=win, name='block_info',
    text='Machen Sie eine Pause. \n\nBitte denken Sie daran, sich während der Anzeige des Hinweises auf das Fixierungskreuz zu konzentrieren. \n\nDrücken Sie die LEERTASTE, um fortzufahren...',
    font='Arial',
...
calibrationTarget = visual.TargetStim(win, 
    ...
    radius=0.35, fillColor=[0.6549, 0.6549, 0.6549], borderColor='white', lineWidth=1.0,
    innerRadius=0.14, innerFillColor='green', innerBorderColor='white', innerLineWidth=1.0,
...
# --- Prepare to start Routine "synchronize" ---
...
synchronizeComponents = [text, key_sync]
for thisComponent in synchronizeComponents:
...
while continueRoutine:
    ...
    # if key_sync is starting this frame...
    if key_sync.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        key_sync.frameNStart = frameN  # exact frame index
        key_sync.tStart = t  # local t and not account for scr refresh
        key_sync.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(key_sync, 'tStartRefresh')  # time at next scr refresh
        # add timestamp to datafile
        thisExp.timestampOnFlip(win, 'key_sync.started')
        # update status
        key_sync.status = STARTED
        # keyboard checking is just starting
        waitOnFlip = True
        win.callOnFlip(key_sync.clock.reset)  # t=0 on next screen flip
        win.callOnFlip(key_sync.clearEvents, eventType='keyboard')  # clear events on next screen flip
    if key_sync.status == STARTED and not waitOnFlip:
        theseKeys = key_sync.getKeys(keyList=['space'], waitRelease=False)
        _key_sync_allKeys.extend(theseKeys)
        if len(_key_sync_allKeys):
            key_sync.keys = _key_sync_allKeys[-1].name  # just the last key pressed
            key_sync.rt = _key_sync_allKeys[-1].rt
...
# --- Ending Routine "synchronize" ---
for thisComponent in synchronizeComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
# check responses
if key_sync.keys in ['', [], None]:  # No response was made
    key_sync.keys = None
thisExp.addData('key_sync.keys',key_sync.keys)
if key_sync.keys != None:  # we had a response
    thisExp.addData('key_sync.rt', key_sync.rt)
thisExp.nextEntry()
# the Routine "synchronize" was not non-slip safe, so reset the non-slip timer
routineTimer.reset()
...
# update component parameters for each repeat
# keep track of which components have finished
et_record_startComponents = [etRecord]
...
# update component parameters for each repeat
# keep track of which components have finished
et_record_stopComponents = [etRecord_2]
